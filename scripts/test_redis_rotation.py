...
def test_redis_rotation() -> None:
    ...
    if len(real_keys) < 2:
        fail("Requires at least 2 real Gemini API keys in .env (e.g. GEMINI_API_KEY_1, GEMINI_API_KEY_2) to test rotation.")
        return
    ...
    call_count = 0
    quota_error = Exception("resource exhausted — 429")
